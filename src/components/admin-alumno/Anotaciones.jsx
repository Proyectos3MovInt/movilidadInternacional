"use client";

export default function Anotaciones() {
  return (
    <div className="bg-white rounded-[0.5rem] p-6 w-full">
      <h3 className="text-[#0065EF] font-semibold mb-2 text-sm">Anotaciones</h3>
      <p className="text-[#14192C] text-sm leading-[1.25rem]">
        Lorem ipsum dolor sit amet consectetur. Sit congue tristique amet eu
        nisi montes. Fames vulputate dapibus aliquam sem tincidunt. Malesuada
        ipsum amet laoreet nisi tortor donec mollis sed malesuada.
      </p>
    </div>
  );
}
