...
const MenuSuperior = ({ searchTerm, setSearchTerm }) => {
  ...
  return (<div className="w-[1440px] h-16 relative">
    ...
    <button className="w-48 h-8 relative cursor-pointer" onClick={() => router.push("/admin-dashboard")}>
        <Icons.LogoUtad />
      </button>
      <div className="flex justify-end items-center gap-5">
        <div data-property-1="buscador corto" className="w-10 h-10 relative cursor-pointer">
        ...
        <div onClick={() => router.push("/admin-calendar")} data-property-1="Default" className="px-4 py-1 bg-blue-600 rounded-lg flex justify-start items-center gap-2 cursor-pointer">
          <button className="w-6 h-6 relative cursor-pointer">
            <Icons.CalendarHeader />
          </button>
          <div className="justify-start text-white text-base font-normal font-['Montserrat'] leading-normal">Calendario</div>
        </div>
      </div>
    </div>
  </div>
